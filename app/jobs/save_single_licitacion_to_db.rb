...
class SaveSingleLicitacionToDB
  ...
  def self.check_if_unchanged(latest_result, prev_latest_result, codigo_externo)
    ...
    # was created
    latest = remove_fecha_creacion(latest_result)
    previous_latest = remove_fecha_creacion(prev_latest_result)
    return if latest == previous_latest
  ...
  def self.after_create_result_actions(result, prev_result_value, cod_externo,
    ...
    if result.save
      ...
      # -> no notifs to create
      unless @previous_result_value.nil?
  ...
  def self.get_previous_result_value(codigo_externo)
    ...
    previous_result_value = last_result.value unless last_result.nil?
    previous_result_value
  end
