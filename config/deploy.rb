## Change these
server (ENV['CHILECOMPRA_RAILS_SERVER_IP']).to_s, user: (ENV['CHILECOMPRA_RAILS_DEPLOY_USER']).to_s, port: 22, roles: [:web, :app, :db], primary: true

set :repo_url,        (ENV['CHILECOMPRA_RAILS_REPO']).to_s
set :application,     (ENV['CHILECOMPRA_RAILS_APP_NAME']).to_s
set :user,            (ENV['CHILECOMPRA_RAILS_DEPLOY_USER']).to_s
...
set :ssh_options,     forward_agent: true, user: fetch(:user), keys: %W(#{ENV['CHILECOMPRA_RAILS_KEY_LOCATION']})
